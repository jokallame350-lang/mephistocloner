...
async function scrapeUrl(targetUrl, options = {}) {
  ...
  // Resilient Serverless Edge Fallback (Vercel / Cloud Functions without local Chrome)
  if (!chromePath) {
    console.log(`[Scraper] Local Chrome not detected. Executing resilient serverless edge extractor for ${url}...`);
    ...
    try {
      ...
      console.warn(`[Scraper] Edge fetch warning: ${fetchErr.message}. Utilizing offline fallback...`);
...
async function analyzeRawHtml(html, css = '', options = {}) {
  ...
  if (!chromePath) {
    const { analyzeOfflineFallback } = require('./cloud-scraper-connector');
    return analyzeOfflineFallback(html, css, 'local://raw-input');
  }
