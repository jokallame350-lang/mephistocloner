...
function filterDomStructure(domStr, detailLevel) {
  if (!domStr) return '(Semantic DOM tree not supplied)';
  const lines = domStr.split('\n');

  if (detailLevel === 'compact') {
    // Keep shallow depth (up to 2 indentation levels) and limit to 40 lines
  ...
  if (detailLevel === 'balanced') {
    // Keep up to 4 indentation levels and limit to 120 lines
  ...
  // Exhaustive: full hierarchy up to 400 lines
...
function formatExtractedCss(fullCSS, detailLevel) {
  if (!fullCSS || !fullCSS.trim()) {
    return '/* No extracted raw CSS supplied - synthesize based on design tokens above */';
  }

  if (detailLevel === 'compact') {
    // In compact mode, omit or provide minimal snippet
    return '/* Extracted CSS omitted in compact mode — follow design tokens and layout rules */';
  }

  if (detailLevel === 'balanced') {
    // Balanced: up to ~5,000 characters
    if (fullCSS.length > 5000) {
      return fullCSS.slice(0, 5000) + '\n/* ... [truncated for balanced token budget] ... */';
    }
    return fullCSS;
  }

  // Exhaustive: up to 20,000 characters
  if (fullCSS.length > 20000) {
    return fullCSS.slice(0, 20000) + '\n/* ... [truncated at 20,000 char limit] ... */';
  }
  return fullCSS;
}
