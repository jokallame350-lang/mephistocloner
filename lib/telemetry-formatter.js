...
function formatAssets(images, assetMode, detailLevel) {
  ...
  const imgLimit = isCompact ? 4 : (detailLevel === 'balanced' ? 8 : 20);

  let formattedImgs = images.imgs.slice(0, imgLimit).map((img, i) => {
    let src = img.src || '';
    if (src.startsWith('data:')) {
      src = '[INLINE_IMAGE_ASSET]';
    } else if (src.length > 120) {
      src = src.slice(0, 115) + '...';
    }
    ...
    const altTag = img.alt ? ` | alt="${img.alt.slice(0, 40)}"` : '';
    return `  - ${roleTag}${src}${dims}${altTag}`;
  });

  let formattedBgs = images.bgImages.slice(0, isCompact ? 2 : 4).map(bg => {
    if (bg.startsWith('data:')) return '  - [INLINE_BACKGROUND_ASSET]';
    if (assetMode === 'placeholders') {
    ...
    return `  - ${bg.length > 120 ? bg.slice(0, 115) + '...' : bg}`;
  });
  ...
  return {
    ...
    svgSamples: images.svgSamples.slice(0, isCompact ? 1 : 3),
    iconLinks: images.iconLinks.slice(0, 3),
  };
}
...
function formatCssVariables(cssVars, detailLevel) {
  ...
  if (detailLevel === 'compact') {
    // Filter to top 6 most relevant variables (colors, fonts, radii)
    const priorityVars = entries
      .filter(([k]) => /color|primary|bg|background|font|radius|shadow/i.test(k))
      .slice(0, 6);
    const selected = priorityVars.length ? priorityVars : entries.slice(0, 6);
  ...
  // Exhaustive
  return entries.slice(0, 25).map(([k, v]) => `  ${k}: ${v};`).join('\n');
}
...
function formatColors(colors, detailLevel) {
  ...
  const limit = detailLevel === 'compact' ? 6 : (detailLevel === 'balanced' ? 10 : 20);
...
function formatTypography(typography, fonts, detailLevel) {
  ...
  if (typoEntries.length) {
    const limit = detailLevel === 'compact' ? 4 : (detailLevel === 'balanced' ? 8 : 15);
  ...
  const families = fonts.families.length
    ? fonts.families.slice(0, detailLevel === 'compact' ? 3 : 6).join('\n  • ')
    : 'System sans-serif stack (-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif)';

  const sizes = fonts.sizes.length
    ? fonts.sizes.slice(0, detailLevel === 'compact' ? 6 : 10).join(' | ')
    : '12px | 14px | 16px | 18px | 20px | 24px | 32px | 48px';

  let fontFacesText = '/* Use standard Google / Web font imports */';
  if (detailLevel === 'exhaustive' && fonts.fontFaces) {
    fontFacesText = fonts.fontFaces.slice(0, 400);
  }
  ...
  return {
    ...
    families: `  • ${families}`,
    sizes: `  ${sizes}`,
    links: fonts.links.length ? fonts.links.slice(0, 2).map(l => `  - ${l}`).join('\n') : '  (none)',
    fontFaces: fontFacesText,
  };
}
...
function formatLayout(layout, spacing, detailLevel) {
  const isCompact = detailLevel === 'compact';
  const gridLimit = isCompact ? 2 : (detailLevel === 'balanced' ? 4 : 8);
  const flexLimit = isCompact ? 2 : (detailLevel === 'balanced' ? 5 : 10);
  const spacingLimit = isCompact ? 2 : (detailLevel === 'balanced' ? 5 : 10);

  const grids = layout.grids.slice(0, gridLimit).map(g =>
    `  ${g.el || 'container'} — columns: ${g.cols || 'auto'} | gap: ${g.gap || 'none'}`
  ).join('\n');

  const flexboxes = layout.flexboxes.slice(0, flexLimit).map(f =>
    `  ${f.el || 'container'} — direction: ${f.direction || 'row'} | justify: ${f.justify || 'flex-start'} | align: ${f.align || 'stretch'}`
  ).join('\n');

  const spacingBlock = spacing.slice(0, spacingLimit).map(s =>
    `  ${s.el || 'section'} — padding: ${s.padding || '0'} | max-width: ${s.maxWidth || 'none'}`
  ).join('\n');

  return {
    sections: layout.sections.slice(0, 6).join(' | ') || 'header | main | footer',
    grids: grids || '  (standard responsive CSS grid)',
    flexboxes: flexboxes || '  (standard flexbox)',
    spacing: spacingBlock || '  (standard Tailwind spacing)',
  };
}
...
function buildTelemetrySections(rawTelemetry, options = {}) {
  ...
  const isCompact = detailLevel === 'compact';
  const scriptLimit = isCompact ? 1 : (detailLevel === 'balanced' ? 3 : 8);
  const styleLimit = isCompact ? 1 : (detailLevel === 'balanced' ? 3 : 8);

  const externalScripts = data.external.scripts.length
    ? data.external.scripts.slice(0, scriptLimit).map(s => `  - ${s.length > 90 ? s.slice(0, 85) + '...' : s}`).join('\n')
    : '  (none)';
  const externalStyles = data.external.styles.length
    ? data.external.styles.slice(0, styleLimit).map(s => `  - ${s.length > 90 ? s.slice(0, 85) + '...' : s}`).join('\n')
    : '  (none)';
