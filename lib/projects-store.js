...
const PLAN_CONFIGS = {
  free: {
    ...
    promptsPerDay: 3,
    priceMonthly: 0,
    features: ['150 Günlük Kredi (3 Site Analiz Hakkı)', 'React 19 & Vanilla HTML Çıktısı', 'Babel Canlı Sandbox', 'Topluluk Desteği']
  },
  pro: {
    id: 'pro',
    name: 'Pro Developer',
    ...
    promptsPerDay: 12,
    priceMonthly: 19,
    features: ['600 Günlük Kredi (12 Site Analiz Hakkı)', 'Claude 3.7 & GPT-4o Canlı Akış', 'Tüm 5 Framework Desteği', '1-Click GitHub & Vercel Dağıtım', 'Çoklu Sayfa Tarayıcı']
  },
  agency: {
    id: 'agency',
    name: 'Agency & Scale',
    ...
    promptsPerDay: 30,
    priceMonthly: 79,
    features: ['1,500 Günlük Kredi (30 Site Analiz Hakkı)', 'Sınırsız Kendi API Anahtarın (BYOK)', 'Öncelikli SLA & Hızlı Sunucular', 'Sınırsız Çalışma Alanı']
  }
};
...
function getUser(userId = 'usr_guest_default') {
  ...
  user.remainingPrompts = Math.floor(user.credits / 50);
...
/**
 * Deduct credits for a generation (default 50 credits per site prompt)
 */
function deductCredits(userId, amount = 50) {
  const user = getUser(userId);
  ...
  if (user.credits < amount) {
    return {
      ...
      message: 'Kredi limiti tükendi! Günlük 3 ücretsiz analiz hakkınızı (150 kredi) kullandınız.',
  ...
  return {
    ...
    credits: user.credits,
    creditsLimit: user.creditsLimit,
    remainingPrompts: Math.floor(user.credits / 50),
    timeUntilResetMs: user.timeUntilResetMs,
    nextReset: user.nextReset
  };
}
