...
async function crawlMultiPage(targetRootUrl, options = {}) {
  ...
  const { findChromeExecutable } = require('./stealth-scraper');
  const chromePath = options.chromePath || findChromeExecutable();

  // Resilient Serverless Edge Fallback
  if (!chromePath && !browser) {
    console.log(`[Crawler] Local Chrome not detected. Falling back to serverless edge multi-page crawler for ${rootUrl}...`);
    ...
    try {
      ...
      const rootTelemetry = analyzeOfflineFallback(rootHtml, '', rootUrl);
      return aggregateMultiPageTelemetry(rootTelemetry, {});
    } catch (err) {
      console.warn(`[Crawler] Edge crawl warning: ${err.message}`);
      const fallbackTelemetry = analyzeOfflineFallback(`<html><head><title>${rootUrl}</title></head><body><h1>${rootUrl}</h1></body></html>`, '', rootUrl);
      return aggregateMultiPageTelemetry(fallbackTelemetry, {});
    }
  }
